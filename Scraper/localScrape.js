...
const localScrape = async () => {
    ...
    const localNews = await page.evaluate(() => {
        ...
        document.querySelectorAll('.mh-content .mh_magazine_posts_grid .mh-posts-grid').forEach((el) => {
            ...
            el.querySelectorAll('.mh-posts-grid-col').forEach((elem) => {
                ...
                let title = elem.querySelector('article.mh-posts-grid-item h3.entry-title a').textContent.trim()
                let link = elem.querySelector('article.mh-posts-grid-item h3.entry-title a').getAttribute('href')
                let img = elem.querySelector('article.mh-posts-grid-item figure.mh-posts-grid-thumb a.mh-thumb-icon img').getAttribute('src')
                let category = elem.querySelector('article.mh-posts-grid-item figure.mh-posts-grid-thumb div.mh-image-caption').textContent.trim()
                let body = elem.querySelector('article.mh-posts-grid-item div.mh-posts-grid-excerpt div.mh-excerpt p').textContent.trim()
                let date = elem.querySelector('article.mh-posts-grid-item div.entry-meta span.entry-meta-date a').textContent
...
if(fs.existsSync(path.join(__dirname,'localNews.json'))){
    ...
    fs.writeFile(path.join(__dirname,'localNews.json'), JSON.stringify(newNews), err => {
